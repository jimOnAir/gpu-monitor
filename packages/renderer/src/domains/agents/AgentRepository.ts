...
export class AgentRepository {
  ...
  async fetchGpus(agentUrl: string): Promise<IGpu[] | null> {
    ...
    try {
      logger.debug('AgentRepository', undefined, `Fetching GPUs from ${agentUrl}`);
      const response = await fetch(`${agentUrl}/gpu`, {
  ...
  async healthCheck(agentUrl: string): Promise<boolean> {
    try {
      logger.debug('AgentRepository', undefined, `Health check: ${agentUrl}`);
      const controller = new AbortController();
